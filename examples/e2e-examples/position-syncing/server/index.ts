import { Connection, pino, Room, RoomManager, Server } from '@isaacmason/rapida-server';
import { CLIENT_ENDPOINT, SCENE_ID, TESTING_ROOM_ID } from 'common';
import { PlayerPositionUpdateEvent, PLAYER_POSITION_UPDATE } from 'common/events/client-to-server';
import { OtherBoxesUpdateEvent, OTHER_BOXES_UPDATE_EVENT, PlayerInitEvent, PLAYER_INIT_EVENT } from 'common/events/server-to-client';
import { PlayerConnectionEvent, PLAYER_CONNECTION_EVENT } from '../common/events/server';
...
class ExampleRoomManager extends RoomManager {
  players: { [id: string]: { id: string, position: { x: number, y: number, z: number } } } = {};
  ...
  init = () => {
    this.room.addHandler(PLAYER_POSITION_UPDATE, 'updatePlayerPosition', (event: PlayerPositionUpdateEvent) => {
